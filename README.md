...
## 10. (Optional) Noise-robustness experiment

Sibling of §5 that quantifies degradation of a raw-trained vs precise-trained
model on precise test data, per-pair Δθ displacement, and PELT+ segmentation
stability under noise. Requires **two** extracted pair sets (raw and precise)
bound into one manifest.

```bash
# 1) extract raw and precise pair sets from the two I-80 CSV versions
python extract_cf_pairs.py ngsim -i <RAW_I80_csv>     --n-train 100 --n-test 20 --min-duration 30 -o cf_ngsim_I80_raw
python extract_cf_pairs.py ngsim -i <PRECISE_I80_csv> --n-train 100 --n-test 20 --min-duration 30 -o cf_ngsim_I80_precise

# 2) bind the same (follower, leader) pairs across both roots
python merge_robustness_pairs.py \
    --raw-root cf_ngsim_I80_raw --precise-root cf_ngsim_I80_precise \
    --out robustness_manifest.csv

# 3) degradation + Δθ + segmentation stability (+ optional synthetic σ-sweep)
python run_robustness.py \
    --manifest robustness_manifest.csv --model idm \
    --features s_end,dist,phase_speed_ols \
    --sigma-sweep "0.0,0.1,0.2,0.3" \
    --out robustness_out
```

**Outputs** (`robustness_out/`): `robustness_summary.json`,
`degradation_per_pair.csv`.
