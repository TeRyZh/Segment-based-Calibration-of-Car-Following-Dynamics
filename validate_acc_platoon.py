...
IDM is free-run against the observed leader and, closed-loop, down the platoon.
  ...
  (1) PLATOON string instability  [primary]  -- closed-loop chain; cumulative
      ...
      (Gamma > 1 == string unstable). Observed vs sample-sim vs phase-sim, so the
...
python validate_acc_platoon.py --pairs-dir oa_cf_pairs --calib-dir acc_calib_ovm_run7 -o acc_string_stability --run 7 --hysteresis
...
def amplitudes(P):
    ncar = len(P["labels"])
    A_o = [float(np.ptp(P["v_obs"][i])) for i in range(ncar)]
    A_s = [float(np.ptp(P["v_sim"][i])) for i in range(ncar)]
    sd_o = [float(np.std(P["v_obs"][i])) for i in range(ncar)]
    sd_s = [float(np.std(P["v_sim"][i])) for i in range(ncar)]
...
def plot_platoon(run_id, P_ref, amps, sims, which_list, out_png, win_s):
    t = P_ref["t"]; i0, w = _busiest_window(t, P_ref["v_obs"][-1], win_s); sl = slice(i0, i0 + w)
    tt = t[sl] - t[i0]; ncar = len(P_ref["labels"]); pos = np.arange(1, ncar + 1)
    colors = plt.cm.plasma(np.linspace(0.08, 0.9, ncar))
    primary = "phase" if "phase" in which_list else which_list[0]
    ...
    # panel 2: cumulative amplitude A_i (observed + each objective)
    ...
    ax2.set_title("Cumulative disturbance amplitude down the platoon")
...
def platoon_metric_rows(run_id, P_ref, amps, which_list) -> List[dict]:
    rows = []
    for i, lab in enumerate(P_ref["labels"]):
        row = {"run": run_id, "position": i + 1, "make": lab,
               ...
               "mean_speed_ms": float(np.mean(P_ref["v_obs"][i]))}
        for which in which_list:
...
def run(args) -> None:
    ...
    for run_id, rjobs in by_run.items():
        ...
        sims, amps = {}, {}
        for which in which_list:
            P = build_platoon(model, rjobs, which, warmup_frames)
            sims[which] = P
            amps[which] = amplitudes(P)
        P_ref = sims[which_list[0]]
        plat_rows.extend(platoon_metric_rows(run_id, P_ref, amps, which_list))
    ...
    if plat_rows:
        ...
        print("String amplification (observed vs objectives):")
...
def _selftest(args) -> None:
    ...
    for p, mk, *_ in specs:
        json.dump({"model": "idm", "follower_make": mk, "follower_position": p,
                   ...
                   "theta_sample": th, "theta_phase": th2},
    ...
    amps = {w: amplitudes(sims[w]) for w in which_list}
    plot_platoon("syn", sims[which_list[0]], amps, sims, which_list,
                 os.path.join(args.output_dir, "platoon_syn.png"), args.window)
    for f in ["phase_decomp_f2_Car_B_sample.png", "phase_decomp_f2_Car_B_phase.png",
              "trace_f2_Car_B.png", "hysteresis_f2_Car_B_phase.png", "platoon_syn.png"]:
        assert os.path.exists(os.path.join(args.output_dir, f)), f"missing {f}"
    assert sum(1 for p in prows if p["objective"] == "phase") == 2, "phase rows"
    print(f"\nSelf-test PASS: both objectives validated, platoon overlays sample+phase, "
          f"figures + metrics written to {args.output_dir}")
 
 
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="ACC platoon string-instability validation comparing "
                    "sample-based vs phase-anchored calibrations.",
    ...
    p.add_argument("-o", "--output-dir", default="acc_validation")
    p.add_argument("--which", choices=["sample", "phase", "both"], default="both",
    ...
    p.add_argument("--window", type=float, default=120.0,
                   help="Busiest-window length (s) for platoon/trace figures.")
